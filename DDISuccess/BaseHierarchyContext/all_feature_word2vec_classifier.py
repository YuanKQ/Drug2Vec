...
from utils import permute_dataset, fully_connected_layer, convolutional_layer, lenet5, BiRNN
...
train_head_word2vec = train_data[:, feature_dim:feature_dim+100]
train_tail_data     = train_data[:, feature_dim+100:dim_x-100]
train_tail_word2vec = train_data[:, dim_x-100:]
test_head_data     = test_data[:, 0:feature_dim]
test_head_word2vec = test_data[:, feature_dim:feature_dim+100]
test_tail_data     = test_data[:, feature_dim+100:dim_x-100]
test_tail_word2vec = test_data[:, dim_x-100:]
# print("dim_x: ", dim_x, "\ntrain_size:", train_data.shape[0], "\ntest_size:",test_data.shape[0])
print("train_head_data: ", train_head_data.shape, "train_head_word2vec: ", train_head_word2vec.shape)
print("train_tail_data: ", train_tail_data.shape, "train_tail_word2vec:" , train_tail_word2vec.shape)
print("test_head_data: ",  test_head_data.shape,  "test_head_word2vec: ", test_head_word2vec.shape)
print("test_tail_data: ",  test_tail_data.shape,  "test_tail_word2vec: ", test_tail_word2vec.shape)
...
head_word2vec_placeholder = tf.placeholder(tf.float32, [batch_size, 100])
...
tail_word2vec_placeholder = tf.placeholder(tf.float32, [batch_size, 100])
...
# input_data = tf.reshape(tf.concat([head_trans, head_word2vec_placeholder, head_hierarchy_placeholder,
#                                    tail_trans, tail_word2vec_placeholder, tail_hierarchy_placeholder], 1),
#                         [batch_size, latent_dim*2+hierarchy_dim*2+100*2, 1, 1])
input_data = tf.reshape(tf.concat([head_trans, head_word2vec_placeholder,
                                   tail_trans, tail_word2vec_placeholder, ], 1),
                        [batch_size, latent_dim*2+100*2, 1, 1])
...
with tf.Session() as sess:
    for epoch in range(epochs):
        ...
        runner.train_model(train_op, result.loss, train_batch_num,
                           feed_vars=(head_data_placeholder, head_word2vec_placeholder, head_hierarchy_placeholder,
                                      tail_data_placeholder, tail_word2vec_placeholder, tail_hierarchy_placeholder, labels_placeholder),
                           feed_data=pt.train.feed_numpy(batch_size,
                                                         train_head_data, train_head_word, train_head_hierarchy,
                                                         train_tail_data, train_tail_word, train_tail_hierarchy, train_labels),
                           print_every=500)
        ...
        classification_accuracy = runner.evaluate_model(accuracy, test_batch_num,
                                                        feed_vars=(head_data_placeholder, head_word2vec_placeholder, head_hierarchy_placeholder,
                                                                   tail_data_placeholder, tail_word2vec_placeholder, tail_hierarchy_placeholder, labels_placeholder),
                                                        feed_data=pt.train.feed_numpy(batch_size,
                                                                                      test_head_data, test_head_word, test_head_hierarchy,
                                                                                      test_tail_data, test_tail_word, test_tail_hierarchy, test_labels),
                                                        print_every=500)
