...
def lstm_classifier(labels, x, timesteps, per_example_length, phase=pt.Phase.train):
    ...
    lstm = input.cleave_sequence(timesteps).sequence_lstm(500).sequence_lstm(100)
    return (lstm.squash_sequence()
            .dropout(keep_prob=0.8, phase=phase)
            .fully_connected(32, activation_fn=None).softmax_classifier(2, labels))
...
with tf.variable_scope("drug_classifier"):
    train_result = lstm_classifier(labels_placeholder, input_data, time_steps, vector_dim)
...
with tf.variable_scope("drug_classifier", reuse=True):
    result = lstm_classifier(labels_placeholder, input_data, time_steps, vector_dim, phase=pt.Phase.test)
