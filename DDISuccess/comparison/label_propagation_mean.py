...
@Description: 基于药物特征判断药物之间的相互作用关系， 作用效果再取平均值
"""
import sys
...
def build_lp_dataset(relations, features_dict, dim_name):
    ...
    for i in idx:
        ...
        tail = relations[i][1]
        rel = relations[i][2]
        feature_matrix.append(np.concatenate((features_dict[head][dim_name], features_dict[tail][dim_name])))
        feature_matrix.append(np.concatenate((features_dict[tail][dim_name], features_dict[head][dim_name])))
        if rel == "increase":
            lab = 0
        else:
            lab = 1
...
def get_classfier_result(y_true, y_pred, y_logit):
    ...
    # print(y_logit)
    y_logit = tf.nn.softmax(y_logit)
    _, auroc_tensor = tf.metrics.auc(y_logit, y_true_matrix)
    _, aupr_tensor = tf.metrics.auc(y_logit, y_true_matrix, curve="PR")
    ...
    print("------------------------")
    return accuracy, precision[0], recall[0], fscore[0], auroc, aupr
...
def label_propagation(train_x, train_y, test_x, test_y):
    ...
    return get_classfier_result(test_y, y_pred, y_logit)
...
if __name__ == '__main__':
    ...
    for name in dim_names:
        print("feature: ", name)
        increase_feature_matrix, increase_labs = build_lp_dataset(ddi_increase, features_dict, name)
        decrease_feature_matrix, decrease_labs = build_lp_dataset(ddi_decrease, features_dict, name)
        ...
        results.append(label_propagation(train_x, train_y, test_x, test_y))
