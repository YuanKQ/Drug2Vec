...
def load_numpy(path, binarize_y=False):
    ...
    f = gzip.open(path, 'rb')
    train, valid, test = pickle.load(f, encoding='unicode-escape') # https://blog.csdn.net/qq_18433441/article/details/60569298
    f.close()
...
# Loads train_dataset where train_dataset is split into class labels
...
# Create semi-supervised sets of labeled and unlabeled train_dataset
...
def create_semisupervised(x, y, n_labeled):
    ...
    for i in range(n_classes):
        ...
        random.shuffle(idx)
        x_labeled[i] = x[i][:, idx[:n_labels_per_class]]  #截取每种类别的前n_labels_per_class个样本
