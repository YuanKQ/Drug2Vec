...
class Conv2dGenerativeClassifier(object):
    ...
    def __init__(self,
        ...
        with self.G.as_default():
            self.x_labelled_mu = tf.placeholder(tf.float32, [self.num_lab_batch, self.dim_x])
            self.x_labelled_lsgms = tf.placeholder(tf.float32, [self.num_lab_batch, self.dim_x])
            self.x_unlabelled_mu = tf.placeholder(tf.float32, [self.num_ulab_batch, self.dim_x])
            self.x_unlabelled_lsgms = tf.placeholder(tf.float32, [self.num_ulab_batch, self.dim_x])
            self.y_lab = tf.placeholder(tf.float32, [self.num_lab_batch, self.dim_y])

            self.classifier = ConvolutionallyConnected(dim_output=self.dim_y,
                                             hidden_layers=hidden_layers_qy)

            self.encoder = ConvolutionallyConnected(dim_output=2 * self.dim_z,  # 隐变量空间的均值和方差
                                          hidden_layers=hidden_layers_qz)

            self.decoder = ConvolutionallyConnected(dim_output=2 * self.dim_x,  # mean_dim + var_dim = dim_x
    ...
    def _generate_yx(self, x_mu, x_log_sigma_sq, phase=pt.Phase.train, reuse=False):
        ...
        with tf.variable_scope('classifier', reuse=reuse):  # 权重共享
    ...
    def _objective(self):
        ...
        for label in range(self.dim_y):
            ...
            if label == 0:
                ...
                L_ulab = tf.concat([L_ulab, _L_ulab], 1)
        ...
        with self.session as sess:
            ...
            for epoch in range(epochs):
                ...
                if epoch % print_every == 0:
                    ...
                    assert x_valid_mu.shape[0] % self.num_lab_batch == 0, '#Valid % #Batches != 0'
                    x_valid_batch_count = x_valid_mu.shape[0] // self.num_lab_batch
                    print("valid: self.num_lab_batch: ", self.num_lab_batch)
                    # print("y_")
                    for x_valid_mu_batch, x_valid_lsgms_batch, y_valid_batch in pt.train.feed_numpy(self.num_lab_batch, x_valid_mu, x_valid_lsgms, y_valid):
                        tmp_accuracy, tmp_cross_entropy = sess.run([self.eval_accuracy, self.eval_cross_entropy],
                                                                  feed_dict={self.x_labelled_mu   : x_valid_mu_batch,
