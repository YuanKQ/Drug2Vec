...
from M2.conv2d.conv2d_genclass import Conv2dGenerativeClassifier
from M2.conv2d.conv2d_vae import Conv2dVariationalAutoencoder


import time

from M2.ddi_train_vae import train_ddi_vae
...
def VAE_encode_dataset(model_path, dataset, factor_dim, dim_z, M1_hidden_layers_px, M1_hidden_layers_qz):
    batch_size = 100
    VAE = Conv2dVariationalAutoencoder(dim_x=factor_dim, dim_z=dim_z, batch_size=batch_size,
...
def ddi_classifier(VAE_model_path, min_std, num_lab, dim_z, M1_hidden_layers_px, M1_hidden_layers_qz,
    ...
    ''' Load Dataset '''
    ####################
    test_dataset_path = "/data/cdy/ykq/ddi_dataset/test_dataset"
    ...
    def encode_dataset(model_path, facter_dim, dim_z, min_std=0.0, ):
        # VAE = Conv2dVariationalAutoencoder(dim_x=facter_dim, dim_z=dim_z,
        #                              hidden_layers_px=M1_hidden_layers_px,
        #                              hidden_layers_qz=M1_hidden_layers_qz)  # Should be consistent with model being loaded
        # with VAE.session:
        #     VAE.saver.restore(VAE.session, model_path)

        enc_x_lab_mean, enc_x_lab_var = VAE_encode_dataset(model_path, x_lab, facter_dim, dim_z, M1_hidden_layers_px, M1_hidden_layers_qz)
        enc_x_ulab_mean, enc_x_ulab_var = VAE_encode_dataset(model_path, x_ulab, facter_dim, dim_z, M1_hidden_layers_px, M1_hidden_layers_qz)
        enc_x_valid_mean, enc_x_valid_var = VAE_encode_dataset(model_path, x_valid, facter_dim, dim_z, M1_hidden_layers_px, M1_hidden_layers_qz)
        enc_x_test_mean, enc_x_test_var = VAE_encode_dataset(model_path, x_test, facter_dim, dim_z, M1_hidden_layers_px, M1_hidden_layers_qz)
    ...
    GC = Conv2dGenerativeClassifier(  dim_x, dim_z, dim_y,
    ...
    GC_eval = Conv2dGenerativeClassifier(  dim_x, dim_z, dim_y, num_examples, num_lab, num_batches,
                                     hidden_layers_px= M2_hidden_layers_px,
                                     hidden_layers_qz= M2_hidden_layers_qz,
                                     hidden_layers_qy= M2_hidden_layers_qy,
                                     )
...
if __name__ == '__main__':
    ...
    VAE_model_path = '/data/cdy/ykq/checkpoints/conv2d/checkpoints/model_VAE_500_06-05-082850.cpkt'
    M1_hidden_layers_px = [500]
    M1_hidden_layers_qz = [500]
    M2_hidden_layers_px = [500]
    M2_hidden_layers_qz = [500]
    M2_hidden_layers_qy = [500]
    ...
    ddi_classifier(VAE_model_path, min_std, num_lab, dim_z, M1_hidden_layers_px, M1_hidden_layers_qz,
                   M2_hidden_layers_px, M2_hidden_layers_qz, M2_hidden_layers_qy)
    print("elapse:%gh" % ((time.time() - start)/3600))
