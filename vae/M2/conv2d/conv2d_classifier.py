...
from ddi.data_process import permute_dataset, load_dataset, load_dataset_split
import numpy as np
...
if __name__ == '__main__':
    ...
    learning_rate = 3e-4
    num_labs = [28700]
    with open("conv2d_results.txt", "w+") as wf:
        for num_lab in num_labs:
            print("train_size is {}.".format(num_lab))
            accuracy, epoch, final_accuracy = main_process(num_lab,num_batches, learning_rate, epochs)
            wf.write("{}\t{}\t{}\n".format(num_lab, epoch, accuracy, final_accuracy))